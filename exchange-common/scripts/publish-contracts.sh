...
copy_errors() {
    local version="$1"
    local out_dir="$2"
    local errors_source="$CONTRACTS_DIR/versions/$version/errors.md"
    local dest="$out_dir/errors.md"

    if [ ! -f "$errors_source" ]; then
        echo "Warning: errors.md not found at $errors_source; skipping."
        return 0
    fi

    if [ -e "$dest" ] && [ "$errors_source" -ef "$dest" ]; then
        return 0
    fi

    cp "$errors_source" "$dest"
    echo "  Copied Errors: errors.md"
}
...
main() {
    ...
    copy_errors "$version" "$output_dir"
