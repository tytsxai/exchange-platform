...
### 2.2 Trade（成交）
- 主键：`tradeId`（全局唯一，跨 symbol 递增，便于审计与回放）
- 建议字段：
...
### 2.9 ApiKey（API Key）
- 主键：`apiKeyId`
- 唯一：`apiKey`（公开部分）唯一；secret 只存加密密文（或 hash）
- 字段：`userId, apiKey, encryptedSecret, permissions, ipWhitelist, status, createdAtMs, updatedAtMs`
- 说明：权限最小化；创建/修改/删除必须审计；建议使用 `API_KEY_SECRET_KEY` 做对称加密（如 AES-GCM），密文可加版本前缀（如 `enc:v1:`）
