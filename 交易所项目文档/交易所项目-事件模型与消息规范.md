...
#### 4.1.1 订单状态机（建议最小集）
- `INIT -> NEW`（内部准备态：落库后冻结资金/风控校验通过再进入 NEW）
...
> 私有推送用于向用户实时推送订单/成交/余额变化，通过 Gateway 的 `/ws/private` 端点提供。

### 7.1 推送通道
- Redis Pub/Sub 通道模板：`private:user:{userId}:events`
- Gateway 订阅用户通道后，将事件转发到对应的 WebSocket 连接

### 7.2 私有事件类型
- `order.update`：订单状态变化（NEW/PARTIALLY_FILLED/FILLED/CANCELED/REJECTED）
- `trade.created`：用户成交回报（包含 maker/taker 双方）
- `balance.update`：余额变化（冻结/解冻/清算后）
...
私有 WS 连接时需携带签名参数：
```
ws://gateway:8090/ws/private?apiKey=<apiKey>&timestamp=<timestampMs>&nonce=<nonce>&signature=<signature>
```

签名算法与 REST API 一致（见接口契约文档）。

### 7.5 活性检测
- Gateway 的 `/ready` 会检查 `privateEventsConsumer` 活性
- 如果 Redis Pub/Sub 消费循环异常，`/ready` 返回 `degraded`/`503`
