...
- ID：`orderId/tradeId/ledgerId` 等均为字符串（避免前端/JS 精度问题）。
...
### 4.1.3 私有鉴权（示例）
```json
{"op":"auth","apiKey":"***","timestampMs":1730000000000,"nonce":"***","signature":"***"}
```
...
### 4.3 私有频道（建议）
- `user.orders`（订单回报：NEW/PARTIAL/FILLED/CANCELED/REJECTED）
- `user.balances`（资产变化）
...
- `RATE_LIMIT`：可重试（退避 + 尊重 `Retry-After`）
