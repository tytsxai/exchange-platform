...
### 1.1 共享工具库（exchange-common/pkg）

| 包 | 功能 |
|------|------|
| `decimal` | 高精度计算（int64 缩放） |
| `errors` | 统一错误码 |
| `snowflake` | 雪花 ID 生成 |
| `signature` | API 签名验证（HMAC-SHA256） |
| `redis` | Redis Streams 封装 |
| `logger` | 结构化日志（zerolog + traceID） |
| `health` | 健康检查（Live/Ready/Health 端点） |
| `tracing` | 链路追踪（OpenTelemetry + Jaeger） |
| `validate` | 参数验证（Symbol/Price/Quantity/Address） |
| `audit` | 审计日志（异步写入、敏感参数脱敏） |
...
| exchange-admin | 8087 | exchange-admin | RBAC、交易对配置、Kill Switch、审计日志 |
| exchange-wallet | 8086 | exchange-wallet | 充值地址、链上扫描、提现状态机 |
...
- [x] 参数验证增强（symbol 格式、userID 边界）- 已实现 `pkg/validate`
- [x] 结构化日志（替换 log.Printf）- 已实现 `pkg/logger`（zerolog）
- [ ] 配置外部化（移除硬编码测试密钥）
- [ ] 数据库连接池配置
- [x] 优雅关闭超时处理 - 各服务已增强 graceful shutdown
...
- [ ] Kubernetes 部署配置
- [x] 分布式追踪集成 - 已实现 `pkg/tracing`（OpenTelemetry + Jaeger）
- [ ] 告警规则配置
- [ ] 自动化备份脚本
